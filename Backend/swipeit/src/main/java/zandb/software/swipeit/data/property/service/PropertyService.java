...
public class PropertyService {
    ...
    public PropertyDTO updateProperty(long propertyId, PropertyDTO propertyDTO) throws UserIsNotSupplierException, PropertyNotFoundException, PropertyDoesNotBelongToLoggedInUser {
    ...
    public void deleteProperty(long propertyId) throws UserIsNotSupplierException, PropertyNotFoundException, PropertyDoesNotBelongToLoggedInUser {
        getIdOfLoggedInUserAndCheckIfPropertyCanBeUpdatedOrDeleted(propertyId);

        propertyRepository.deleteById(propertyId);
    }

    private long getIdOfLoggedInUserAndCheckIfPropertyCanBeUpdatedOrDeleted(long propertyId) throws UserIsNotSupplierException, PropertyNotFoundException, PropertyDoesNotBelongToLoggedInUser {
        if (!swipeItUserDetailsService.getTypeOfLoggedInUser().equals(UserType.SUPPLIER.getType())) {
            throw new UserIsNotSupplierException("This user is no supplier and can therefore not manipulate properties!");
        }

        if (!propertyRepository.existsById(propertyId)) {
            throw new PropertyNotFoundException("The property with the id %d doesn't exist!".formatted(propertyId));
        }
        ...
        if (oldProperty.getOwner().getUserId() != id) {
            throw new PropertyDoesNotBelongToLoggedInUser("The property with the id %d does not belong to the logged in user and can therefore not be update".formatted(id));
        }
        return id;
    }
}
